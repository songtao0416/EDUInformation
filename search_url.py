...
# 打开url，搜索keyword，获取当前页面的href，返回urls
    ...
    while True:
        ...
        for k1 in soup.find_all('h3', class_='c-title'):  # 资讯中class为'c-title'，网页中class为't'
            try:
                ...
                print("获取url,erreo")
        for k2 in soup.find_all('p', class_='c-author'):
            try:
                k = k2.text
                t_pattern = re.compile(r"(\d{4}\S\d{1,2}\S\d{1,2})")
                a_time = re.search(t_pattern, ''.join(k)).group(0)
                if a_time == 'null':
        ...
        i = i + 1
        try:
            # if len(driver.find_element_by_id("page").find_elements_by_link_text(str(i + 1))) == 0:
            if i == 2:  # 获取前五页，约50篇
                break
                # 如果存在下一页，则继续翻页，否则，结束函数
                html = driver.page_source
                soup = BeautifulSoup(html, 'lxml')
                print("正在获取尾页内容")
                # 读取页面内所有a标签中的href
...
def search_url(search_type, url_adds,mkpath):
    ...
    (a_urls,a_titles,a_times,count) = open_url(search_type)  #根据关键词获取所有搜索页面的url
